...
class PairwiseDifferenceBase(sklearn.base.BaseEstimator):
    ...
    def pair_input(X1, X2):  # -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        try:
            ...
            #! The differentiation between tab data and NLP data is to be made here, because the difference calculates differently
            # calculate_difference = pNLP.get_pair_diff(x1_pair,x2_pair)
            # go over x1, x2 in double for loop, calc difference for each text

        except:
            raise ValueError(
                "PairwiseDifference: The input data is not compatible with the subtraction operation. Either transform all data to numeric features or use a ColumnTransformer to transform the data."
            )
    ...
    def pair_output_difference(
        y1: pd.Series, y2: pd.Series, nb_classes: int
    ) -> pd.Series:
        """For MultiClassClassification base on difference only"""
        y_pair = pd.DataFrame(y1).merge(y2, how="cross")
        y_pair_diff = (y_pair.iloc[:, 1] != y_pair.iloc[:, 0]).astype(int)
        assert (
            y_pair_diff.nunique() <= 2
        ), f"should only be 0s and 1s {y_pair_diff.unique()}"
        return y_pair_diff
    ...
    def check_output(y: pd.Series) -> None:
        ...
        assert (
            isinstance(y, pd.Series) or y.shape[1] == 1
        ), f"Didn't expect more than one output {y.shape}"
    ...
    @staticmethod
    def check_estimator(
        estimator, expecting_classifier=False, expecting_regressor=False
    ) -> None:
        try:
            ...
            if isinstance(estimator, sklearn.base.BaseEstimator):
                ...
                if expecting_classifier:
                    ...
                    if "class_weight" in inspect.signature(
                        estimator.__class__.__init__
                    ).parameters and (
                        estimator.class_weight is None
                        or estimator.class_weight != "balanced"
                    ):
                        warnings.warn(
                            'For better performance, estimator should have class_weight="balanced".'
                        )
                ...
                if isinstance(estimator, incompatible_estimators):
                    warnings.warn(
                        "PDL is not compatible with base estimators of type parametric models, i.e., even if the code works there is low chance of improvement compared to using directly that estimator. To obtain an improvement, it is better to use a tree-based model like: ExtraTrees, RandomForest, DecisionTree, Bagging, etc."
                    )
            else:
                warnings.warn(
                    "Make sure the estimator has a .fit() and .predict() methods."
                )
        except:
            return
    ...
    @staticmethod
    def correct_sample_weight(
        sample_weight: pd.Series, y_train: pd.Series
    ) -> pd.Series:
...
class PairwiseDifferenceClassifier(
    ...
    def fit(
        ...
        if self.check_input:
            ...
            # # this turn it to ndarray # todo put back
        ...
        y_pair_diff = PairwiseDifferenceBase.pair_output_difference(
            self.y_train_, self.y_train_, self.nb_classes_
        )
    ...
    def predict_similarity_samples(
        self, X: pd.DataFrame, X_anchors=None, reshape=True
    ) -> pd.DataFrame:
        ...
        assert isinstance(predictions_proba_difference, np.ndarray), type(
            predictions_proba_difference
        )
        np.testing.assert_array_equal(
            predictions_proba_difference.shape, (len(X_pair), 2)
        )
        ...
        predictions_proba_similarity = (
            predictions_proba_similarity_ab + predictions_proba_similarity_ba
        ) / 2.0
        if not reshape:
    ...
    def predict_samples(self, X: pd.DataFrame) -> pd.DataFrame:
        raise NotImplementedError(
            "todo use predict_proba_samples to get the prediction here"
        )
    ...
    def predict_proba_samples(self, X: pd.DataFrame) -> np.ndarray:
        ...
        if not isinstance(X, pd.DataFrame):
            X = pd.DataFrame(X)
        predictions_proba_similarity_df: pd.DataFrame = self.predict_similarity_samples(
            X
        )
        ...
        def g(anchor_class: np.ndarray, predicted_similarity: np.ndarray) -> np.ndarray:
            ...
            likelyhood_per_anchor = (
                (1 - predicted_similarity) / (1 - self.prior[anchor_class])
            )[:, np.newaxis] * self.prior
            likelyhood_per_anchor[
                np.arange(len(likelyhood_per_anchor)), anchor_class
        ...
        def f(predictions_proba_similarity: np.ndarray) -> np.ndarray:
            ...
            test_i_trains_classes = g(
                ...
                predicted_similarity=predictions_proba_similarity,
            )
            np.testing.assert_array_equal(
                test_i_trains_classes.shape, (len(self.y_train_), self.nb_classes_)
            )
            # test_i_trains_classes is part of the 3d array that I will not return for now
            np.testing.assert_array_almost_equal(test_i_trains_classes.sum(axis=1), 1.0)
            return test_i_trains_classes

        tests_trains_classes_likelihood = np.apply_along_axis(
            f, axis=1, arr=predictions_proba_similarity_df.values
        )  # todo  guess this is slow, get rid of it

        np.testing.assert_array_equal(
            tests_trains_classes_likelihood.shape,
            (len(X), len(self.y_train_), self.nb_classes_),
        )
        np.testing.assert_array_almost_equal(
            tests_trains_classes_likelihood.sum(axis=-1), 1.0
        )
        return tests_trains_classes_likelihood

    @staticmethod
    def _apply_weights(
        tests_trains_classes_likelihood: np.ndarray, sample_weight: np.ndarray
    ) -> np.ndarray:
        tests_classes_likelihood = (
            tests_trains_classes_likelihood * sample_weight[np.newaxis, :, np.newaxis]
        ).sum(axis=1)
        np.testing.assert_array_almost_equal(tests_classes_likelihood.sum(axis=-1), 1.0)
    ...
    def predict_proba(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.decide_use_prior():
            ...
            assert (
                tests_classes_likelihood.max() < 1.00001
            ), tests_classes_likelihood.max()
            assert (
                tests_classes_likelihood.min() > -0.00001
            ), tests_classes_likelihood.min()
            ...
            assert np.isclose(
                tests_classes_likelihood.sum(axis=1), 1, rtol=1e-15, atol=5 * eps
            ).all(), tests_classes_likelihood.sum(axis=1)

            tests_classes_likelihood = tests_classes_likelihood.clip(0, 1)
            if input_type is np.ndarray:
                return tests_classes_likelihood
            tests_classes_likelihood_df = pd.DataFrame(
                tests_classes_likelihood, index=X.index
            )
            ...
            if self.sample_weight_ is not None:
                ...
                def f(predictions_proba_similarity: pd.Series) -> pd.Series:
                    df = pd.DataFrame(
                        {
                            "start": self.y_train_.reset_index(drop=True),
                            "similarity": predictions_proba_similarity,
                        }
                    )
                ...
                if self.proba_aggregate_method == "norm":
                    tests_classes_likelihood_np = (
                        tests_classes_likelihood_np.values
                    ...
                    np.testing.assert_almost_equal(
                        np.sum(tests_classes_likelihood_np, axis=-1), 1.0
                    )
    ...
    def score_difference(self, X, y) -> float:
        ...
        y_pair_diff = PairwiseDifferenceBase.pair_output_difference(
            y, self.y_train_, self.nb_classes_
        )  # 0 if similar, 1 if diff
...
class PairwiseDifferenceRegressor(
    ...
    def __init__(self, estimator: sklearn.base.RegressorMixin = None):
        """
        ...
        self._name_to_method_mapping = {
            ...
            "L2": functools.partial(
                self._sample_weight_optimize, l2_lambda=0.1
            ),  # Recommended method
    ...
    def _to_pandas(*args):
        return (
            (
                data
                if data is None or isinstance(data, (pd.DataFrame, pd.Series))
    ...
    def _pair_data(X1, X2, y1=None, y2=None):
        ...
        assert not is_unsigned_integer_dtype(
            y1
        ), "_pair_data()'s input y1 is an unsigned integer."
        assert not is_unsigned_integer_dtype(
            y2
        ), "_pair_data()'s input y2 is an unsigned integer."
        ...
        if y1 is not None:
            assert (
                isinstance(y1, pd.Series) or y1.shape[1] == 1
            ), f"Didn't expect more than one output {y1.shape}"
            assert (
                isinstance(y2, pd.Series) or y2.shape[1] == 1
            ), f"Didn't expect more than one output {y2.shape}"
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series, sample_weight=None, check_input=True):
        ...
        X_pair, _, y_pair_diff = self._pair_data(
            self.X_train_, self.X_train_, self.y_train_, self.y_train_
        )
        assert (
            y_pair_diff.abs().max() <= 2 * self.y_train_.abs().max()
        ), f"should be a-(-a) <= 2*a Expected:\n{y_pair_diff.abs().max()} <=  {2 * self.y_train_.abs().max()}"
        # todo add assert on y_pair_diff: min<0  , max>0 and dtype float not uint
        self.estimator.fit(X_pair, y_pair_diff)
        return self

    def _predict_samples(
        self, X: pd.DataFrame, force_symmetry=True
        ...
        assert isinstance(predictions_difference, np.ndarray), type(
            predictions_difference
        )
        if force_symmetry:
            difference_sym: np.ndarray = self.estimator.predict(X_pair_sym)
            predictions_difference = (predictions_difference - difference_sym) / 2.0
        ...
        prediction_samples_df = pd.DataFrame(
            predictions.reshape((-1, len(self.X_train_))),
            index=X.index,
            columns=self.X_train_.index,
        )
        # The predicted difference to the anchors:
        pred_diff_samples_df = pd.DataFrame(
            predictions_difference.reshape((-1, len(self.X_train_))),
            index=X.index,
            columns=self.X_train_.index,
        )
    ...
    def predict(self, X, force_symmetry=True) -> pd.Series:
        ...
        prediction_samples_df, _ = self._predict_samples(
            X, force_symmetry=force_symmetry
        )
        ...
        if self.sample_weight_ is None:  # No weights
            ...
            np.testing.assert_array_almost_equal(self.sample_weight_.sum(axis=1), 1.0)
        ...
        assert not any(
            pd.isna(prediction_stats)
        ), f"Prediction should not contain Nans\n{prediction_samples_df}\n{prediction_stats}"
    ...
    def learn_anchor_weights(
        ...
        **kwargs,
        ...
        if method not in self._name_to_method_mapping.keys():
            raise NotImplementedError(
                f"Weighting method {method} unknown! Use one of the following:"
                f" '{', '.join(list(self._name_to_method_mapping.keys()))}'"
            )

        sample_weight: pd.Series = self._name_to_method_mapping[method](
            X_val=X_val, y_val=y_val, X_test=X_test, **kwargs
        )
        assert (
            not sample_weight.isna().any()
...
class PDCDataTransformer(sklearn.base.BaseEstimator, sklearn.base.TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        X, _ = cast_uint(X)
        self.preprocessing_ = get_generic_preprocessing(
            self.numeric_features, self.ordinal_features, self.string_features
        )
    ...
    def transform(self, X, y=None):
        ...
        if any(
            x in pd.Series(X.values.flatten()).apply(type).unique()
            for x in (
                "csr_matrix",
                "date",
            )
        ):  # todo think about adding  'str'
